...
* Rebalance the propagandanda side quest so that more cards are played each turn. Hopefully we can see the player win, and maybe they can keep winning for a few days.
* Added two new modifiers to propaganda posters: Superficial and Thought-Provoking.
* Reworded player advantage to indicate it's not so good for you if you win by delaying enough time.
