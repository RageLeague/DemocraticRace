...
function SupportScreen:init( owner, on_end_fn )
    ...
    self.candidates = self.content:AddChild(DemocracyClass.Widget.CandidateEntryList())
        :SetWidth(content_width)
...
function SupportScreen:Refresh(new_mode)
    ...
    self.candidates:Refresh(new_mode)
...
function SupportScreen:OnScreenModeChange( sm )
    ...
    self.candidates:SetWidth(content_w)
...
function SupportScreen:GetDefaultFocus()
    return (self.general_support and self.general_support:GetDefaultFocus())
        or (self.candidates and self.candidates:GetDefaultFocus())
...
function SupportScreen:Layout()
    ...
    self.general_support:LayoutBounds("left", "below", self.subtitle)
        :Offset( 0, -SPACING.M1 )
    self.candidates:LayoutBounds("left", "below", self.general_support)
        :Offset( 0, SPACING.M1 )
    self.faction_support:LayoutBounds("left", "below", self.candidates)
        :Offset( 0, SPACING.M1 )
